...
const LoginPage = () => {
  ...
  async function handlingLogin(e) {
    ...
    try {
      const response = await fetch(`${api}/api/admin/check/credentials`, {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    <div className="w-full min-h-screen bg-slate-50 flex flex-col lg:flex-row antialiased">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 overflow-hidden items-center justify-center">
        ...
        <div className="relative z-10 px-12 text-white max-w-lg space-y-4">
          <span className="bg-white/10 text-white backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold tracking-wide uppercase border border-white/20 inline-flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-[#058b7c]" /> Admin Portal
          </span>
          <h2 className="text-3xl sm:text-4xl font-bold leading-tight">
            Centralized Platform Governance.
          </h2>
          <p className="text-slate-300 text-sm leading-relaxed">
            Monitor healthcare workflows, oversee registered medical professionals, manage patient safety metrics, and system analytics.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-slate-200/80 shadow-sm">
          ...
          <div className="space-y-3 text-center">
            ...
            <p className="text-xs text-slate-500">
              Enter your elevated credentials to access administrative controls.
            </p>
          </div>

          {/* Form */}
          <form onSubmit={handlingLogin} noValidate className="space-y-5">
            {/* Email Field */}
          ...
          <div className="text-center pt-2">
            <p className="text-xs text-slate-400">
              Restricted area. Unauthorized access attempts are logged and monitored.
            </p>
          </div>
        </div>
      </div>
